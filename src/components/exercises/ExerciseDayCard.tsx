  ...
  return (
    <div>
      <div
        ...
        className={`border rounded-lg p-2 md:p-4 bg-surface hover:bg-surface-hover cursor-pointer max-w-xs flex flex-col space-y-2 ${
          errors?.["exercises"] ? "border-red-500" : "border-border"
        }`}
      >
      ...
      {errors?.["exercises"] && (
        <p className="text-red-500 text-sm mt-1 wrap-anywhere">{errors["exercises"]}</p>
      )}
    </div>
  );
};
